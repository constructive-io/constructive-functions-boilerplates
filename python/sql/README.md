...
This is a **sql** feature, written in python: it is db-connected.

`await ctx.db(fn)` runs the callback in one transaction that has assumed a
low-privilege role and stamped the invocation's identity claims, so the tenant's
RLS applies to every statement — it commits on return, rolls back on raise. The
...
feature (`fun init <name> --surface gql --lang python`), whose context has no
`db`.
...
The context is the same surface a TypeScript handler gets — `ctx.db`,
`ctx.secrets`, `ctx.storage`, `ctx.agent`, `ctx.log`, `ctx.job` — because it is
the same runtime, mirrored in python rather than reimplemented. The declared
inputs are compiled once, at generation, into the JSON Schema both languages
enforce, so a payload the node runtime would refuse is refused here too.
